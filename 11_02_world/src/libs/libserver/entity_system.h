...
public:
    ...
    template <class T, typename... TArgs>
    T* AddComponentWithParent(IEntity* pParent, TArgs... args);
...
{
    return AddComponentWithParent<T>(nullptr, std::forward<TArgs>(args)...);
}

template <class T, typename ... TArgs>
T* EntitySystem::AddComponentWithParent(IEntity* pParent, TArgs... args)
{
    auto pCollector = _systemManager->GetPoolCollector();
    auto pPool = dynamic_cast<DynamicObjectPool<T>*>(pCollector->GetPool<T>());
    T* pComponent = pPool->MallocObject(_systemManager, pParent, 0, std::forward<TArgs>(args)...);
...
{
    auto pComponent = ComponentFactory<TArgs...>::GetInstance()->Create(_systemManager, className, sn, std::forward<TArgs>(args)...);
    if (pComponent == nullptr)
        return nullptr;
